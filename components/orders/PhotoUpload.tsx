...
export function PhotoUpload({ orderId, canEdit }: PhotoUploadProps) {
  ...
  async function handleDelete(photo: OrderPhoto) {
    ...
    await supabase.storage.from('product-photos').remove([photo.file_path])
    await supabase.from('order_photos').delete().eq('id', photo.id)
    setDeletingId(null)
    await fetchPhotos()
  }

  function canDeletePhoto(photo: OrderPhoto): boolean {
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <span className="text-sm font-medium text-[#0f1b35]">{tr.productPhotos}</span>
        {canEdit && (
          <button
            type="button"
